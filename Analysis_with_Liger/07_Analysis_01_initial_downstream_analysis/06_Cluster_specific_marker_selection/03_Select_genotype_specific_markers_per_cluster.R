...
temp_files = str_c(list.files(DEG_dir, full.names = TRUE), "/")

if (!dir.exists(str_c(storing_dir, "/", "Specific_markers_between_genotypes_per_cluster"))){
  dir.create(str_c(storing_dir, "/", "Specific_markers_between_genotypes_per_cluster"), showWarnings = TRUE, recursive = FALSE, mode = "0777")
}

# Create a folder to store the markers file
temp_holder = str_c(storing_dir, "/", "Specific_markers_between_genotypes_per_cluster", "/")
...
for (i in c(1:length(temp_files))){
  ...
  temp_df = list.files(temp_files[i], full.names = TRUE)
  
  for (j in c(1:length(temp_df))) {
    cluster_deg = loadRData(temp_df[j])
    ...
    write.csv(specific_markers, file = str_c(temp_dir, sub(pattern = "\\.[^.]+$", replacement = "", x = basename(temp_df[j])), "_specific_markers.csv"), row.names = TRUE)
  }
}
