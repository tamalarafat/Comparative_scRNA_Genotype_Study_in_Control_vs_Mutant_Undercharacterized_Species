...
# Storing directory
res_dir = "/netscratch/dep_tsiantis/grp_laurent/tamal/2023/Analyses/comparative_study_of_wt_and_mutant_genotype_Cardamine/Liger_Analyses/Liger_analysis_strategy_1/On_coefficient/Analysis_with_CC"
...
for (i in c(1:length(n_neighbors))){
  for (j in c(1:length(n_seed))){
    integrated.data <- RunUMAP(integrated.data, reduction = "inmf", dims = 1:ncol(integrated.data@reductions$inmf@cell.embeddings), n.components = 2, n.neighbors = n_neighbors[i], seed.use = n_seed[j])
